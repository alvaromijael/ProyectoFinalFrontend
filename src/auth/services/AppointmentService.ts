...
class AppointmentService {
  ...
  static async createAppointment(appointmentData: AppointmentCreate): Promise<ApiResponse<Appointment>> {
    try {
      ...
      return this.handleError(error, 'Error al crear la cita');
    }
  }
  ...
  static async getAppointments(params: SearchParams = {}): Promise<ApiResponse<Appointment[]>> {
    try {
      const { skip = 0, limit = 100, include_patient = true } = params;
      ...
      return this.handleError(error, 'Error al obtener las citas');
    }
  }
  ...
  static async getAppointmentById(appointmentId: string | number): Promise<ApiResponse<Appointment>> {
    try {
      ...
      return this.handleError(error, 'Error al obtener la cita');
    }
  }
  ...
  static async getTodayAppointments(): Promise<ApiResponse<Appointment[]>> {
    try {
      ...
      return this.handleError(error, 'Error al obtener las citas de hoy');
    }
  }
  ...
  static async getUpcomingAppointments(params: UpcomingAppointmentsParams = {}): Promise<ApiResponse<Appointment[]>> {
    try {
      const { skip = 0, limit = 100, days_ahead = 7 } = params;
      const response = await api.get<Appointment[]>(
      ...
      return this.handleError(error, 'Error al obtener las próximas citas');
    }
  }
  ...
  static async searchAppointments(params: AppointmentSearchParams): Promise<ApiResponse<Appointment[]>> {
    try {
      ...
      return this.handleError(error, 'Error al buscar citas');
    }
  }
  ...
  static async getAppointmentsByDateRange(params: DateRangeParams): Promise<ApiResponse<Appointment[]>> {
    try {
      ...
      return this.handleError(error, 'Error al obtener citas por rango de fechas');
    }
  }
  ...
  static async getAppointmentsByStatus(status: string, params: SearchParams = {}): Promise<ApiResponse<Appointment[]>> {
    try {
      ...
      return this.handleError(error, 'Error al obtener citas por estado');
    }
  }
  ...
  static async getAppointmentsByPatient(patientId: string | number, params: SearchParams = {}): Promise<ApiResponse<Appointment[]>> {
    try {
      ...
      return this.handleError(error, 'Error al obtener las citas del paciente');
    }
  }

  // Contar citas de un paciente
  static async getAppointmentCountByPatient(patientId: string | number): Promise<ApiResponse<{ patient_id: number; appointment_count: number }>> {
    try {
      const response = await api.get<{ patient_id: number; appointment_count: number }>(
      ...
      return this.handleError(error, 'Error al contar las citas del paciente');
    }
  }
  ...
  static async updateAppointment(appointmentId: string | number, appointmentData: AppointmentUpdate): Promise<ApiResponse<Appointment>> {
    try {
      const response = await api.put<Appointment>(`/appointments/${appointmentId}`, appointmentData);
      return {
        success: true,
        data: response.data,
      ...
      return this.handleError(error, 'Error al actualizar la cita');
    }
  }

  // Eliminar cita
  static async deleteAppointment(appointmentId: string | number): Promise<ApiResponse<{ message: string }>> {
    try {
      ...
      return this.handleError(error, 'Error al eliminar la cita');
    }
  }
